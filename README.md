...
| employee_no | TEXT | 従業員コード（従業員マスタの person_code・7桁。2026-09-18 に旧社員番号から置き換え。CSV では列名「社員番号」のまま）|
| sort_order | INT | ソート順 |
...
| 記録 | 日別の打刻記録一覧・修正（日付/区分/時刻）・削除・氏名検索 |
| 月次CSV | FileMaker・PCA給与DX連携用CSV出力（丸め時間ベース）|
| 個人履歴 | スタッフ別の勤務履歴・サマリー |
| 勤怠一覧 | 全従業員×日付のカレンダーグリッド表示（店舗フィルタ・期間ナビ付き）|
| スタッフマスタ | 従業員の登録・編集・削除（upsert・従業員コードは過去記録にも反映・削除はパスワード保護・勤怠記録がある人は削除不可）・氏名検索・店舗フィルタ・在籍状況フィルタ（在籍中/退職/全て、既定は在籍中）|
...
- 正社員は「通し」区分で全店舗に表示
- CSVはFileMaker・PCA給与DX連携用（1列目は従業員コード＝FM の社員番号フィールド・時間は小数形式・正社員除外・常に丸め適用）
- 従業員保存はupsert方式。勤怠記録がある従業員は削除ボタンの時点で止める（辞めた人は退職日で管理）。念のため保存時も FK 制約のある従業員は削除せず is_active=false にする
- 従業員コード・氏名変更時、過去の打刻記録にも自動反映
- 新しく入った人の従業員コードは、社内ポータル（従業員マスタ）で承認されると **DB トリガが自動で埋める**（氏名の空白を除いた「姓＋名」が1人だけ一致したとき。先にスタッフマスタへ追加してあっても、後から追加しても可）。一致しない・同名が複数のときは手入力
- `employees.employee_no` / `time_records.employee_no` に 7桁のコードが入っている行は、空や旧番号で上書きできない（保護トリガ）。トリガ本体は従業員マスタ側 `hr/従業員マスタ/supabase/migrations/0070_hiyoko_timecard_autolink.sql`（`hr_portal.trg_tc_*`）
